...
def cal_ap_metrics(pred_list, gt_list, thresh_step=0.01):

    final_accuracy_all = cal_prf_metrics(pred_list, gt_list, thresh_step)
    pr_array = np.array(final_accuracy_all)
    
    precision = pr_array[:, 1]
    recall = pr_array[:, 2]
...
def get_image_pairs_separate_folders(gt_dir, pred_dir):


    gt_paths = sorted(glob.glob(os.path.join(gt_dir, '*.png'))) #png
    
    pred_imgs, gt_imgs = [], []
    pred_imgs_names, gt_imgs_names = [], []

    print(f"Found {len(gt_paths)} ground truth images in {gt_dir}")
    ...
    for gt_path in gt_paths:
        ...
        if os.path.exists(pred_path):
            ...
            print(f"Warning: No corresponding prediction found for {gt_path} at {pred_path}")
    
    print(f"Successfully loaded {len(pred_imgs)} image pairs.")
...
def eval(log_eval, results_dir, epoch):
    ...
    return {'epoch': epoch, 'mIoU': mIoU, 'ODS': ODS, 'OIS': OIS, 'AP': AP, 'F1': F_list[0], 'Precision': Precision_list[0], 'Recall': Recall_list[0]}


if __name__ == '__main__':
    gt_image_dir = "datasets/TUT/test/Label_image" #gt folder location
    pred_image_dir = "TUT_prediction_result" # predicted images folder location

    src_img_list, tgt_img_list, pred_imgs_names, gt_imgs_names = get_image_pairs_separate_folders(gt_image_dir, pred_image_dir)
